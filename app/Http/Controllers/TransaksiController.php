...
class TransaksiController extends Controller
{
    ...
    public function store(TransaksiRequest $request)
    {
        ...
        Alert::success('Sukses', "Transaksi {$request->kategori} berhasil");
    ...
    public function cari(CariTransaksiRequest $request)
    {
        ...
        if ($request->kategori == 'transaksi') {
            ...
            $items = Transaksi::with('saldo')->where('kategori',$request->kategori)->whereBetween('tanggal',[$request->tanggal_awal,$request->tanggal_akhir])->get();
        }
    ...
    public function laporanPDF(LaporanTransaksiRequest $request)
    {
        ...
        if ($kategori == 'semua') {
            $items = Transaksi::with('saldo')->whereBetween('tanggal',[$request->tanggal_awal,$request->tanggal_akhir])->get();
            $pdf = PDF::loadView('pages.transaksi.laporan_pdf', ['items' => $items, 'periode' => $periode, 'saldo_terakhir' => $saldo_terakhir]);
            ...
            $pdf = PDF::loadView('pages.transaksi.pemasukan.laporan_pdf', ['items' => $items, 'periode' => $periode]);
            return $pdf->download("Laporan Pemasukan {$periode}");
            ...
            $pdf = PDF::loadView('pages.transaksi.pengeluaran.laporan_pdf', ['items' => $items, 'periode' => $periode]);
            return $pdf->download("Laporan Pengeluaran {$periode}");
        }
    }
}
