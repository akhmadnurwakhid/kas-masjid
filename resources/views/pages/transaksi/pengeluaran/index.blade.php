...
@section('content')
    <div class="card shadow mb-4">
        <div class="card-body">
            <a href="{{route('transaksi.pengeluaran.create')}}" class="btn btn-success mb-4">
               <b>Tambah</b>
                    ...
                    <tbody>
                        ...
                        @forelse ($items as $item)
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-info btn-sm d-inline mr-1 mb-1" href="{{route('transaksi.edit', $item->id)}}">
                                        <i class="fas fa-pencil-alt">
                                        </i>
                                        Ubah
                                    </a>
                                    <form action="{{route('transaksi.destroy', $item->id)}}" method="post" class="d-inline" id="{{'form-hapus-transaksi-'.$item->id}}">
                                        @method('DELETE')
                                        @csrf
                                        <button class="btn btn-danger btn-sm btn-hapus" data-id="{{$item->id}}" data-jumlah="{{ "Rp " . number_format($item->jumlah,2,',','.') }}" type="submit">
...
@push('after-script')
    ...
    <script>
        $('.btn-hapus').on('click', function(e){
            ...
            text: 'Pengeluaran sebesar '+ jumlah +' akan dihapus',
            icon: 'warning',
